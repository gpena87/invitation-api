...
export class EmailService {
  ...
  sendUserUpdateNotification(
    ...
    return from(
      (async () => {
        try {
          const htmlContent = this.generateUpdateEmailHTML(userName, updateDetails);
          const from = this.configService.get<string>('EMAIL_FROM') ?? 'onboarding@resend.dev';
          const { error } = await this.resend.emails.send({
            from,
            to: email,
  ...
  private generateUpdateEmailHTML(
    userName: string,
    updateDetails: Record<string, any>,
  ): string {
    const detailsHTML = Object.entries(updateDetails)
      ...
      <html>
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              <p>Hola ${userName},</p>
              <p>Tu perfil ha sido actualizado con los siguientes detalles:</p>
              <ul>
                ${detailsHTML}
              </ul>
            </div>
            <div class="footer">
              <p>Si no realizaste estos cambios, por favor contacta con soporte.</p>
            </div>
          </div>
        </body>
      </html>
